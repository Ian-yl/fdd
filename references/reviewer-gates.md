...
## Schema 2.2 authoring and review

Under schema 2.2 the scaffold no longer synthesizes business meaning; it emits the mechanical evidence index and capability skeletons in `draft-pending-authoring`. The author agent closes each capability's semantics from the anchored evidence. Determinism moves to the correct side: generation relies on understanding (the agent), verification relies on determinism (the validator and independent reviewer). The validator and the immutable trusted-replay guarantee for approved packages stay exactly as strong.
...
Schema 2.2 review also rejects: a residual `draft-pending-authoring` capability; a `planned` capability without a `missingDecision` record citing which business decision the source leaves unanswered; a non-headless `complete` capability whose trigger control is not bound to an observed release control (control provenance); a field-assist `resultDestination` whose output type is incompatible with its target field; and any indexed evidence item that is neither anchored by an authored closure nor recorded in `evidence-dispositions.json`.
