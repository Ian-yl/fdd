...
- **Resource input is closed.** A file/resource selector resolves to an independently triggered `resourceTransfer` or an explicit controlled-resource reference, and its runtime ID flows into each consuming operation; it is not silently treated as an ordinary page field.
