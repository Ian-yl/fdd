...
For every required request field on a complete UI capability, review confirms one approved origin: a release control, an explicit agent-designed control, application state with rationale and evidence, or a declared cross-operation dependency. Missing artwork alone is not a blocker; an adequately specified `designed-control` is the intended way to extend an incomplete visual baseline. Independent multi-item output whose quantity can exceed one must declare bounded concurrency with `maxParallel >= 2`.
