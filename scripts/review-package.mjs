...
const semanticMode = manifest.schemaVersion === '2.1';
const frontendInventory = semanticMode ? read('frontend-semantic-inventory.json') : {};
const observedInteractions = semanticMode ? read('observed-interactions.json') : {};
const controlMap = semanticMode ? read('control-capability-map.json') : {};
...
for (const capability of spec.capabilities || []) {
  ...
  for (const operation of capability.operations || []) {
    ...
    if (operation.request?.contentType === 'multipart/form-data' && !resourceTransfer) findings.push(`${operation.id}: transfer has no resource transfer contract`);
...
write('review-receipt.json', { schemaVersion: '1.1', contractVersion: `functional-domain/${manifest.schemaVersion}`, validatorDigest: digestFile('validate-package.mjs'), runtimeBundleDigest: runtimeBundleDigest(), status: 'approved', authorAgentId: manifest.authorAgentId, reviewerAgentId, reviewedAt: manifest.approval.reviewedAt, checks: ['all blockers resolved', 'all capability specifications complete', 'evidence statuses reviewed', 'acceptance criteria executable'] });
...
function runtimeBundleDigest() { return hash(['validate-package.mjs', 'review-package.mjs', 'lib/presentation.mjs', 'lib/aggregate-submission.mjs'].map((file) => `${file}\0${readFileSync(resolve(import.meta.dirname, file))}`).join('\0')); }
