...
const frozen = spawnSync(process.execPath, [`${root}/scripts/freeze-validator.mjs`, '--version', '2.2.2', '--check'], { encoding: 'utf8' }); if (frozen.status !== 0) errors.push(frozen.stderr.trim() || frozen.stdout.trim() || 'current validator snapshot differs from generator output');
