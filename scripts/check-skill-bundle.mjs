...
for (const file of ['import-productforge.mjs', 'scaffold-package.mjs', 'review-package.mjs', 'validate-package.mjs', 'build-implementation-handoff.mjs', 'review-implementation-handoff.mjs']) if (digestFile(`${root}/scripts/${file}`) !== digestFile(`${skill}/scripts/${file}`)) errors.push(`scripts/${file} differs from project source`);
for (const file of ['agents/openai.yaml', 'runtime-manifest.json']) if (existsSync(`${root}/${file}`) && existsSync(`${skill}/${file}`) && digestFile(`${root}/${file}`) !== digestFile(`${skill}/${file}`)) errors.push(`${file} differs from project source`);
