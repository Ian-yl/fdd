...
FDD accepts two explicit input modes built from these authority sources:
...
- an optional immutable, digest-valid AI Restore release;
...
`release-backed` uses the architecture, finalized designs, and an immutable AI Restore release. The Agent treats released pages and controls as the implementation baseline.

`design-led` uses the architecture and finalized designs without a frontend release. The scaffold creates an evidence workspace, not inferred UI semantics. The FDD Agent must inspect every selected design with vision and author the page regions, controls, states, intended interactions, and mappings. PI later implements the complete frontend and backend. After PI verification, AI Restore may apply visual restoration only; it does not redefine functionality.

When supplied, the release must contain a valid release manifest, payload manifest, approval, Suite Gate, page results, page restore plans, visual inventories, and publication source. FDD reads it without modification.
