...
- **Resource input is closed.** Local file selection, drop, and replacement use an independent `resourceTransfer`, and its declared response ID flows into consuming operations through `dataDependencies`. An existing-resource selector may use an already controlled resource ID directly. The reviewer checks that the author did not confuse or merge these two workflows.
- **Field assistance is closed.** An action that computes a value for another field has its own processing operation and `resultDestination.targetKind: field` write-back contract; it is not folded into the page submit merely because both controls share a page.

The deterministic validator checks only inventory correspondence, references, method/path agreement, request/response paths, and declared data flow. Whether two observed loops represent one business capability, whether an exclusion rationale is truthful, and whether several same-shaped capabilities are a page template are independent-review judgments. Product orchestration and PI do not repeat this semantic review.
...
Independent review likewise owns input-utilization honesty auditing, symmetric to discard honesty. For a provider-backed capability the reviewing agent reads the anchored source for each `inputUtilization` disposition and confirms it is truthful: an input the source shows must reach the external provider is marked `provider-mapped` with a real mapping (and, for a resource input, a real resolution), not silently downgraded to `application-only` or `not-used`; both alternative dispositions require a reason and evidence anchors. A dishonest disposition — source evidence shows the input feeds the provider, but the ledger drops or diverts it — is rejected with the offending evidence ids. Structural validation guarantees every input carries a disposition with its required mapping, resolution, or evidenced reason; whether each disposition is true is the reviewer's role and does not run in CI.

The audit is per operation, not merely per capability. Review rejects a later provider operation whose fields are absent from the ledger, a request path missing from that operation's schema, or any disagreement among the ledger provider parameter, `parameterMappings`, `integrationBindings`, and requiredness. Domain result assertions are authored only when the source requires content, compliance, textual, usability, or other semantic judgment; their acceptance language remains domain-specific while their execution protocol remains generic.

For every required request field on a complete UI capability, review confirms one approved origin: a release control, an explicit agent-designed control, application state with rationale and evidence, or a declared cross-operation dependency. Missing artwork alone is not a blocker; an adequately specified `designed-control` is the intended way to extend an incomplete visual baseline. The reviewer checks that the authored concurrency choice is suitable for the declared provider and business behavior; tooling only validates and enforces the declaration.
...
Schema 2.3 approvals pin the current repository-owned validator revision. Superseded revisions are not selectable by new approvals or PI preparation; packages must be reviewed and locked against the current revision before implementation.
