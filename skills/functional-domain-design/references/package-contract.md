...
Transfer and integration semantics use `resourceTransfer`, `resourceValidation`, `resourcePersistence`, `integrationBindings`, and `externalEffects`. Local file selection, drop, and replacement use an independent transfer whose returned ID reaches later business operations through `dataDependencies`. Selecting an existing controlled resource is not a file transfer and may bind its resource ID directly. The FDD author and independent reviewer distinguish these cases from release behavior and domain evidence; validators and implementation tools do not infer them from operation IDs, labels, filenames, framework choices, product categories, or vocabulary.
...
Core journeys set `core: true`; all of their capability IDs must resolve to complete capabilities. Server-operation necessity is derived from structured state writes, entity effects, capability-bound permissions/integrations, declared server behavior, and capability side effects. The operation set copied into the implementation handoff is an exact projection of complete FDD operations. If an older approved input exposes a complete server-required capability without an operation, PI may emit a `contract-completion` work item, but an FDD agent must author the missing contract and obtain a new independent review, package lock, handoff review, and handoff lock before implementation proceeds.
