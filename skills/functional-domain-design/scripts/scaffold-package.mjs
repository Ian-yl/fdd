...
writeJSON(`${output}/page-function-map.json`, { schemaVersion: '2.2', pages: pages.map((page) => ({ pageId: page.id, title: page.title, navigationOnly: page.nav === true, capabilityIds: skeletons.filter((item) => item.pageIds.includes(page.id)).map((item) => item.id) })) });
