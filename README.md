...
把三个架构 JSON、不可变前端 release 和可选用户业务决策转换为批准的可执行领域包与 implementation handoff。FDD 内部执行 BMAD 理解、需求分析、领域设计和独立复核，不实现代码、不修改 release。
...
```bash
npm run domain:scaffold -- --input <architecture-input> --visual-release <release> --decisions <optional-decisions.json> --output <functional-domain> --author-agent <id>
